...
abstract class Base {
	 ...
	 *
	 * @var OM4\Zapier\Feed\Feed[] Array of OM4\Zapier\Feed\Feed objects
	 */
	protected $feeds;
	...
	public function __construct() {
		...
		if ( is_null( $this->sort_order ) )
			throw new Exception( 'OM4\Zapier\Trigger\Base::sort_order must be specified' );
	 ...
	 *
	 * Important: the OM4\Zapier\Trigger\Base->is_sample() method should be used to determine whether to use real or sample data.
	 *
	...
	protected function send_to_zapier( $action_name, $arguments ) {
		 ...
		 *
		 * Important: OM4\Zapier\Trigger\Base->is_sample() should be used to distinguish between sample and real data being sent to Zapier.
		 *
		 * @since 1.1.0
		 *
		 * @param array             $data The WooCommerce data about to be sent to Zapier.
		 * @param OM4\Zapier\Trigger\Base $this The Trigger instance that is causing the data to be sent to Zapier.
		...
		foreach ( $this->get_feeds_for_this_trigger() as $feed ) {
			 ...
			 * @param OM4\Zapier\Trigger\Base $this The Trigger instance that caused the data to be sent to Zapier.
			 * @param string            $action_name Hook/action name that initiated this data send to Zapier.
			 * @param array             $arguments Arguments for the hook/action that initiated this data send to Zapier.
			 */
	 ...
	 *
	 * @param OM4\Zapier\Feed\Feed $feed         Feed data
	 ...
	 *
	 * @param OM4\Zapier\Feed\Feed $feed
