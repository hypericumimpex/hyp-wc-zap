...
abstract class Item extends Base {
	...
	final protected static function get_sample_from_json( $class ) {
		...
		array_walk(
			...
			$date_function = function( &$value, $key ) use ( &$date_function ) {
				...
				if ( false !== $position ) {
					$value = date( 'c', time() + (int) $value[1] * 60 * 60 * 24 );
	 ...
	 * @return  bool
	 */
	final protected function equal_type( $key, $value ) {
		...
		// Allow double to equal integer, see issue #188.
		if ( 'double' === $type && is_int( $value ) ) {
			return true;
		}

		// Check scalar and object matches.
		return $value instanceof $type || false !== strpos( gettype( $value ), $type );
	}
